...
exports.PackagePublisher = exports.SharingManager = exports.PublisherManager = exports.GalleryBase = void 0;
const _ = require("lodash");
const colors = require("colors");
const errHandler = require("../../../lib/errorhandler");
const fs = require("fs");
const GalleryInterfaces = require("azure-devops-node-api/interfaces/GalleryInterfaces");
...
class GalleryBase {
     ...
     */
    constructor(settings, galleryClient, extInfo) {
    ...
    static getExtInfo(info) {
        ...
        if (info.extensionId && info.publisher) {
            ...
            promise = new Promise((resolve, reject) => {
                ...
                const extensionId = info.extensionId || _.get(vsixManifestAsJson, "PackageManifest.Metadata[0].Identity[0].$.Id");
                const extensionPublisher = info.publisher || _.get(vsixManifestAsJson, "PackageManifest.Metadata[0].Identity[0].$.Publisher");
                const extensionVersion = _.get(vsixManifestAsJson, "PackageManifest.Metadata[0].Identity[0].$.Version");
                const isPublicExtension = _.get(vsixManifestAsJson, "PackageManifest.Metadata[0].GalleryFlags[0]", []).indexOf("Public") >= 0;
    ...
    extToValidationStatus(extension, version) {
    ...
    getVersionedExtension(extension, version) {
 ...
 */
class PublisherManager extends GalleryBase {
    /**
     * Constructor
     * @param PublishSettings
     */
    constructor(settings, galleryClient) {
     ...
     */
    createPublisher(name, displayName, description) {
        return this.galleryClient
            .createPublisher({
            publisherName: name,
...
class SharingManager extends GalleryBase {
    shareWith(accounts) {
        return this.getExtInfo().then(extInfo => {
...
class PackagePublisher extends GalleryBase {
    checkVsixPublished() {
        return this.getExtInfo().then(extInfo => {
            return this.galleryClient
        ...
        return this.getExtInfo().then(extInfo => {
            ...
            return this.createOrUpdateExtension(this.getExtensionPackage()).then(ext => {
                if (this.settings.noWaitValidation) {
                    ...
                    return this.waitForValidation(1000, validationInterval, validationRetries, hangTightMessageRetryCount, extInfo.publisher, extInfo.id, versions[0].version).then(result => {
    ...
    createOrUpdateExtension(extPackage) {
        return this.checkVsixPublished().then(extInfo => {
            ...
            return publishPromise.then(() => {
                return this.galleryClient.getExtension(null, extInfo.publisher, extInfo.id, null, GalleryInterfaces.ExtensionQueryFlags.IncludeVersions);
            });
        });
    }
    waitForValidation(interval, maxInterval, retries, showPatienceMessageAt, publisher, extensionId, version) {
        if (retries === 0) {
        ...
        return (0, promiseUtils_1.delay)(this.getValidationStatus(version), interval).then(status => {
            ...
            if (status === PackagePublisher.validationPending) {
                ...
                return this.waitForValidation(Math.min(interval * 2, maxInterval), maxInterval, retries - 1, showPatienceMessageAt, publisher, extensionId, version);
            }
            else {
                return status;
            }
